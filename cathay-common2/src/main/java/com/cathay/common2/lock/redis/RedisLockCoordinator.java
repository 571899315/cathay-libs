package com.cathay.common2.lock.redis;

import static com.cathay.cache.redis.JedisProviderFactory.getJedisCommands;
...
import com.cathay.cache.redis.JedisProvider;
import com.cathay.cache.redis.JedisProviderFactory;
import com.cathay.cache.redis.sentinel.JedisSentinelProvider;
import com.cathay.cache.redis.standalone.JedisStandaloneProvider;
...
public class RedisLockCoordinator {
	...
	public RedisLockCoordinator() {	
		...
		String mode = ResourceUtils.getProperty("cathay.lock.redis.mode", ResourceUtils.getProperty("cathay.cache.mode","standalone"));
		String server = ResourceUtils.getProperty("cathay.lock.redis.servers", ResourceUtils.getProperty("cathay.cache.servers"));
		String datebase = ResourceUtils.getProperty("cathay.lock.redis.datebase", ResourceUtils.getProperty("cathay.cache.datebase","0"));
		String password = ResourceUtils.getProperty("cathay.lock.redis.password", ResourceUtils.getProperty("cathay.cache.password"));
		String maxPoolSize = ResourceUtils.getProperty("cathay.lock.redis.maxPoolSize", ResourceUtils.getProperty("cathay.cache.maxPoolSize","100"));
		
		Validate.notBlank(server, "config[cathay.lock.redis.servers] not found");
		...
		if("standalone".equals(mode)){
			...
			String masterName = ResourceUtils.getProperty("cathay.lock.redis.masterName", ResourceUtils.getProperty("cathay.cache.masterName"));
			Validate.notBlank(masterName, "config[cathay.lock.redis.masterName] not found");

...
<dependency>
	...
	<artifactId>cathay-kafka</artifactId>
...
```
    <bean id="topicProducerProvider" class="TopicProducerSpringProvider">
        <!-- 默认是否异步发送 -->
...
###### 首先在spring注册，添加以下配置文件即可
```
<bean  id="topicConsumerProvider" class="TopicConsumerSpringProvider">
        ...
        <property name="topicHandlers" >
            <map>
              ...
              <entry key="demo-topic">
                 <bean class="com.cathay.test.DemoMessageHandler" />
              </entry>
              <entry key="demo2-topic">
                 <bean class="com.cathay.test.Demo2MessageHandler" />
              </entry>
            </map>
        </property>
    </bean>
